...
impl Color {
    pub fn from_hex(hex: u32) -> Self {
        Self((hex & 0xFF) as u8, ((hex & 0xFF00) >> 8) as u8, ((hex & 0xFF0000) >> 16) as u8)
    }
} 
    ...
    pub(crate) fn from_json(json: &Map<String, Value>) -> Self {
        Self {
            ...
            icon_url: if let Some(icon_hash) = json["icon"].as_str(){
                Some(format!("https://cdn.discordapp.com/role-icons/{role_id}/{role_icon}.png", role_id = json["id"].as_str().unwrap(), role_icon = icon_hash))
            } else {
                None
            },
            ...
            permissions: Permission::get_permissions(json["permissions"].as_str().unwrap().parse().unwrap()),
        }
    }
    getter!(&id -> Snowflake);
    getter!(&name -> String);
